...
files_links = pd.read_csv(pdf_links_fpath)
...
bank_col_vals = [bank] * len(todo_files)
time_parsing_complete = [datetime.datetime.now()] * len(todo_files)
todo_files_df = pd.DataFrame(list(zip(bank_col_vals, todo_files, todo_files_links, time_parsing_complete)),
                             columns= ["bank", "filename", "href", "parsed"])
